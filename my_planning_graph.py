...
class ActionLayer(BaseActionLayer):
    ...
    def _competing_needs(self, actionA, actionB):
        ...
        """
        allPairs = product(actionA.preconditions, actionB.preconditions)
        for pair in allPairs: 
            if self.parent_layer.is_mutex(pair[0], pair[1]) or self.parent_layer.is_mutex(pair[1], pair[0]):
                return True
        return False
...
class LiteralLayer(BaseLiteralLayer):
    ...
    def _inconsistent_support(self, literalA, literalB):
        ...
        layers.BaseLayer.parent_layer
        """
        actionsA = self.parents[literalA] # All actions that can lead to literalA
        actionsB = self.parents[literalB] # All actions that can lead to literalB
        actionPairs = product(actionsA, actionsB)
        for actionA, actionB in actionPairs:
            # Check the parent layer to see if (A,B) are mutually exlusive. If there is a collision, return false.
...
class PlanningGraph:
    ...
    def h_setlevel(self):
        ...
        for i, layer in enumerate(self.literal_layers):
            allGoalsMet = True
            for g in self.goal:
                if g not in iter(layer):
                    allGoalsMet = False
                self._extend()
            if not allGoalsMet:
                continue
            goalsAreMutex = False
            for g1, g2 in product(self.goal, repeat=2):
